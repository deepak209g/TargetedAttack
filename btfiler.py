...
class FilerPeer(BTPeer):
    ...
    def __handle_save(self, peerid=None, data=None):
        ...
        if data==None or peerid==None:
            return
        try:
            with open(os.path.join("incoming_msg", "_".join([peerid, uuid.uuid4()])), 'w') as f:
                f.write(data)
                f.close()
        except:
            self.__debug("Failed to save the data")
    ...
    def __handle_fileput(self, peerconn, data):
        # --------------------------------------------------------------------------
        """ Handles the FILEGET message type. The message data should be in
        the format of a string, "file-name", where file-name is the name
        of the file to be fetched.
